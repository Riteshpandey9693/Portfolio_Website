          ...
          <nav className="hidden md:flex space-x-8">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="link-hover text-base font-bold text-white/90 hover:text-white transition-colors duration-200 py-2"
              >
                {link.name}
              </a>
            ))}
        ...
        <div className="flex flex-col items-center justify-center h-full space-y-8">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="text-2xl font-bold text-white/90 hover:text-white transition-colors duration-200"
